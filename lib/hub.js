...
class Hub extends EventEmitter {
  ...
  async process(id, {scanUsed = false,  maxSweeps = constants.MAX_SWEEPS_PER_PROCESS}) {
    ...
    if (processing === null) {
      ...
      res = await this[process](id, scanUsed, maxSweeps);
      resetValue = false;
    }
  ...
  async [process](id, scanUsed, maxSweeps) {
  ...
  async sync(id, maxSweeps = constants.MAX_SWEEPS_PER_SYNC) {
    ...
    if (!this.isAttached(id)) {
      throw new Error(`Hub #${id} is not attached. Attach and process a hub before syncing.`);
    }
    ...
    if (syncing === null) { 
      ...
      res = await this[sync](id, maxSweeps);
      resetValue = false;
    }
  ...
  async [sync](id, maxSweeps) {

    // Fetch array of pending sweeps (not confirmed yet)
    const sweeps = await this[store].lrange(constants.PENDING_SWEEPS_PREFIX, id, 0, --maxSweeps);
      ...
      for (const confirmed of states) {
        ...
        if (confirmed) {
          ...
          for (let j = 0; j < l; j++) {
            ...
            // Release sweep's address, previously locked by process()
            // Prefix `0` indicates that the address is not locked
            const releaseLockedAddress = await this[store].hset(constants.USED_ADDRESSES_PREFIX, id, field, `0_${address}`);
          
            if (!releaseLockedAddress) {
              console.log(`Failed to release locked address ${address} of account #${account} while syncing hub #${id}`);
            }
	        
            // Destination = hotwallet
            const destinationAddress = sweeps[k].destinationAddress;

            // Commit release statement fot hotwallet address, previously locked by [sweep]()
            const commitReleaseStatement = await this[store].decr(constants.HOT_WALLET_LOCKED_INPUTS_PREFIX, destinationAddress);

            if (!commitReleaseStatement && commitReleaseStatement !== 0) {
              console.log(`Failed to commit release statement for locked hot wallet address ${destinationAddress} while syncing hub #{id}`);
            }
  ...
  async [sweep](id, sweeps) {
    ...
    while(sweeps.length) {
      ...
      const numOfSweeps = batch.length;
      ...
      if (transfer) {
        ...
        // Commit lock statement for hot wallet input until sweep confirms
        const commitLockStatement = await this[store].incrby(constants.HOT_WALLET_LOCKED_INPUTS_PREFIX, destinationAddress, numOfSweeps);
        
        if (!commitLockStatement) {
          console.log(`Sweeping for hub #${id} failed. Failed to commit lock statement for hotwallet destination address ${destinationAddress}. Hot wallet input: ${hotWalletInput}`);
          return false;
        }
      ...
      for (const sweep of batch) {
        ...
        if (!saveSweep) {
          console.log(`Could not save sweep from address ${sweep.address} of account #${sweep.account} in hub #${id}. Sweep object: ${sweepObj}`);
        }
    ...
    if (!restoreInput) {
      console.log(`Failed to restore address #${hotWalletInput.address} with key index ${hotWalletInput.index}. Input: ${hotWalletInput}`);  
    }
  ...
  async [credit](hubId, accountId, value, offChain = false) {
    ...
    if (!updatedCredit && updatedCredit !== 0) {
  ...
  async withdraw(hubId, accountId, {address, value, remainderAddress = null, checkAddress = true, tag = ''}) {
    ...
    if (!updatedCredit && updatedCredit !== 0) {
      console.log(`Failed to update account credit while withdrawing value of ${value} for account #${accountId} in hub #${hubId}`);
      return false;
    }    
    ...
    if (!sent) {
      ...
      if (!restoreCredit && restoreCredit !== 0) {
      ...
      for (const restoreInput of collectedInputs) {
        ...
        if (!restoreCollectedInput) {
          console.log(`Failed to restore hotwallet input ${restoreInput.address}. Input: ${restoreInput}`)
        }
    ...
    if (remainder > 0) {
      ...
      if (!saveRemainder) {
        console.log(`Failed to save remainder hot wallet input ${remainderAddress} after withdrawal. Input: ${remainderInput}`);
      }
    ...
    for (let i = 0; i < numNewAddresses; i++) {
      ...
      if (!newAddress) {
        ...
        if (!saveNewAddress) {
          console.log(`Failed to save newly generated hot wallet address to replace consumed input while withdrawing. Input: ${newAddress}`);
        }
  ...
  async [collectHotWalletInputs](value) {
    ...
    while (collectedValue < value) {
      ...
      // If input is locked push it to restore list
      if (lockedInput > 0) {
  ...
  async getAccount(hubId, accountId) {
    ...
    const balanceOnTangle = await this[store].get(constants.ACCOUNT_PREFIX, `${key}_balance`);
    const credit = await this[store].get(constants.ACCOUNT_PREFIX, `${key}_credit`);
    
    if (balanceOnTangle == null || credit == null) {
      return false;
    }
